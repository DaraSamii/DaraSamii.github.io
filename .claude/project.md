...
pages/
  ...
  world.html                — travel map (Leaflet.js + plain OpenStreetMap tiles, no API key — see Components & patterns). Just city markers with click-to-popup; no country highlighting, no side panel (both were tried and explicitly removed). Fullscreen layout: just navbar + map, no header/footer. Travel data is fetched at runtime from assets/data/travels.json (not inline) — the one page with page-specific JS instead of everything living in js/main.js, by deliberate exception (see Conventions & rules).
assets/
  data/
    countries-110m.geojson  — trimmed Natural Earth 110m country boundaries (only NAME + ADM0_A3 properties kept, ~250KB). ORPHANED: originally fetched by pages/world.html to draw a per-country highlight overlay, but that feature was removed at the user's request — nothing in the site fetches this file anymore. Left in place rather than deleted; safe to remove if tidying up, just confirm first.
    travels.json             — the editable list of visited cities for the World page: a flat JSON array of {city, country, countryCode, lat, lng, year, notes}. This is the file to edit to add a new trip — no HTML/JS editing needed. countryCode is currently vestigial (nothing reads it since country highlighting was removed) but harmless to keep. As of the last update covers Canada (mostly Ontario/Quebec + Vancouver/Victoria area), France, Denmark, Italy, Turkey, and Iran.
  images/
...
Third-party lib: **Leaflet.js 1.9.4** (CDN: `cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.{js,css}`) — the only external JS library used anywhere on the site besides Font Awesome's CSS. Tile layer is **plain OpenStreetMap raster tiles** (`{s}.tile.openstreetmap.org/{z}/{x}/{y}.png`) — **no API key required**. The page went through two earlier tile choices before landing here: CartoDB Dark Matter (too low-contrast/hard to navigate) → CartoDB Voyager (needed a CARTO API key, which the user found confusing/unnecessary) → standard OSM (the most "colorful"/standard-looking free option, and drops the API-key dependency entirely). **There is no `CARTO_API_KEY` anymore and no CARTO tile URL** — if you see either in an old note, it's stale.
The page is **markers only — no country highlighting, no info panel**. An earlier version fetched `assets/data/countries-110m.geojson` and drew a green `L.geoJSON` overlay on every visited country plus a `#worldPanel` side card listing cities per country; both were removed at the user's explicit request ("remove the country highlight and the green box", "the detail box should be gone too"). `assets/data/countries-110m.geojson` is now an **orphaned, unused asset** — nothing fetches it anymore; it wasn't deleted, just stopped being referenced (ask before removing it if tidying up later). All that's left per-city is an `L.circleMarker` dot (`--color-accent` fill, `--color-bg` border) with a `bindPopup()` showing city/country/year/notes on click — no separate panel, no country click handler.
Layout is **fullscreen, not a boxed section**: `<body class="world-page">` sets `overflow: hidden`, and the page has no `.page-header`/`.page-content`/`.container`/footer at all — just the navbar followed by a `.world-fullscreen` div (`height: 100vh`) holding `#worldMap` (`.world-map`, `position: absolute; inset: 0`, fills the div completely). Because the navbar is `position: fixed` (floats above content, out of flow), the 100vh map div starts at the true top of the viewport and the translucent/blurred navbar simply floats over it.
**Zoom control**: Leaflet's default `+`/`-` control sits top-left, which the fixed navbar would otherwise sit on top of — the map is created with `zoomControl: false` and a separate `L.control.zoom({ position: 'bottomleft' })` is added instead, restyled in `css/style.css` (`.leaflet-control-zoom a`) to match the dark theme.
**Pan bounds**: `map.setMaxBounds(L.latLngBounds(L.latLng(-85.06, -Infinity), L.latLng(85.06, Infinity)))` restricts *vertical* dragging to where Web-Mercator tiles actually render (past ±85.06° is blank gray — dragging there is now blocked), while longitude is intentionally left unbounded (`Infinity`) so it doesn't fight `worldCopyJump: true`'s seamless east-west wraparound. An earlier version bounded both axes to `[[-85,-180],[85,180]]`, which made horizontal panning feel stuck/janky against `worldCopyJump` — bounding only latitude fixed both the "gray polar area" complaint and the jankiness at once. `maxBoundsViscosity: 1.0` gives a firm (non-bouncy) stop at the lat limits.
Because the map's size is viewport-relative and can change (e.g. mobile browser chrome show/hide, window resize), `initWorldMap()` registers a `window.resize` listener calling `map.invalidateSize()` — a required Leaflet gotcha whenever a container's size comes from CSS/vh rather than fixed pixels set before Leaflet reads it.
Data flow: travel data lives in **`assets/data/travels.json`**, not inline in the page — a flat JSON array of `{city, country, countryCode, lat, lng, year, notes}` objects (`year`/`notes` may be `null`/`""` when unknown; `countryCode` is now vestigial since nothing reads it, but harmless to keep for future use), fetched at runtime via `fetch(TRAVELS_URL)` before the map initializes (`boot()` fetches it, then calls `initWorldMap(travels)`; a fetch failure falls back to `initWorldMap([])` rather than breaking the page). Add a trip by adding an object to that array — no HTML/JS editing needed. Leaflet's own popup/attribution-control chrome is re-themed in `css/style.css` (`.leaflet-popup-*`, `.leaflet-control-attribution`) to match the dark palette instead of Leaflet's default light styling.
