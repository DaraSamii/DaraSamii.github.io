...
Structure: `<body class="world-page">` — this class scopes a set of compact-layout overrides (`.world-page .page-header`, `.world-page .page-content`, `.world-page .container { max-width: 1600px }`) so the map reads as near-fullscreen instead of sitting in the normal large page padding used by every other sub-page. `.world-layout` (flex row, wraps to column ≤768px) contains `#worldMap` (`.world-map`, `height: calc(100vh - 210px)`, min-height 500px; 65vh/min 400px on mobile) and `#worldPanel` (`.world-panel`, fixed 300px sidebar matching the map's height on desktop, capped at 40vh and stacked below the map on mobile). A `.world-stats` line above shows "`N countries · M cities visited`", computed from the `TRAVELS` array. Because the map's height is viewport-relative, `initWorldMap()` also registers a `window.resize` listener calling `map.invalidateSize()` — a required Leaflet gotcha whenever the container's size depends on `vh`/flex rather than a fixed pixel height.
